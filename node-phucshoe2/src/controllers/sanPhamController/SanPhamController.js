  ...
  try {
    ...
    const [products] = await connection.execute(`
      SELECT 
        ...
        GROUP_CONCAT(DISTINCT CONCAT(ms.TEN_MAU_SAC, ' - ', kc.KICH_CO) ORDER BY ms.TEN_MAU_SAC SEPARATOR ', ') AS CHI_TIET_SAN_PHAM,
        GROUP_CONCAT(DISTINCT CONCAT(md.TEN_MUC_DICH_SU_DUNG, ' (', md.CREATE_MUC_DICH_SU_DUNG, ')') ORDER BY md.TEN_MUC_DICH_SU_DUNG SEPARATOR ', ') AS MUC_DICH_SU_DUNG,
        GROUP_CONCAT(DISTINCT pcsp.ID_PHUONG_CACH ORDER BY pcsp.ID_PHUONG_CACH SEPARATOR ', ') AS ID_PHUONG_CACH,
        GROUP_CONCAT(DISTINCT CONCAT(pc.ID_PHUONG_CACH, ': ', pc.TEN_PHONG_CACH) ORDER BY pc.ID_PHUONG_CACH SEPARATOR ', ') AS PHUONG_CACH_CHI_TIET
      FROM SAN_PHAM sp
      LEFT JOIN GIOI_TINH gt ON sp.GIOI_TINH_ID = gt.GIOI_TINH_ID
      LEFT JOIN LOAI_DANH_MUC dm ON sp.ID_DANH_MUC = dm.ID_DANH_MUC
      LEFT JOIN CHAT_LIEU cl ON sp.CHAT_LIEU_ID_ = cl.CHAT_LIEU_ID_
...
const getTop5BestSellingProducts = async (req, res) => {
  try {
    const { ID_NGUOI_DUNG } = req.body; // Lấy ID_NGUOI_DUNG từ query params
    ...
    if (ID_NGUOI_DUNG) {
      // Lấy danh sách ID sản phẩm được người dùng yêu thích
      const [likedProducts] = await connection.execute(
        `
        SELECT ID_SAN_PHAM
        FROM YEU_THICH
      ...
      // Chuyển danh sách ID sản phẩm yêu thích thành một Set để kiểm tra nhanh hơn
      const likedProductIds = new Set(
        likedProducts.map((item) => item.ID_SAN_PHAM)
      );

      // Gắn cờ isLiked cho từng sản phẩm
      const updatedResults = results.map((product) => ({
        ...product,
        isLiked: likedProductIds.has(product.ID_SAN_PHAM), // Kiểm tra sản phẩm có trong danh sách yêu thích
      }));
    ...
    // Trả dữ liệu bình thường nếu không có ID_NGUOI_DUNG
    return res.status(200).json({
      EM: "Xem 5 sản phẩm bán chạy nhất thành công",
      EC: 1,
      DT: results,
...
const get5TopFavoriteProducts = async (req, res) => {
  try {
    ...
    // Truy vấn 5 sản phẩm được yêu thích nhất
    const [results] = await connection.execute(`
      SELECT 
    ...
    if (ID_NGUOI_DUNG) {
      // Truy vấn danh sách sản phẩm mà người dùng đã yêu thích
      const [likedProducts] = await connection.execute(
        `
        SELECT ID_SAN_PHAM
        FROM YEU_THICH
      ...
      // Chuyển danh sách ID sản phẩm yêu thích thành một Set để kiểm tra nhanh
      const likedProductIds = new Set(
        likedProducts.map((item) => item.ID_SAN_PHAM)
      );
...
const getTopExpensiveProducts = async (req, res) => {
  try {
    ...
    // Truy vấn 5 sản phẩm có giá cao nhất
    const [results] = await connection.execute(`
      SELECT 
...
const getProductsUnder20 = async (req, res) => {
  const query = `
    SELECT 
      p.ID_SAN_PHAM,
      p.HINH_ANH_SANPHAM, 
      ...
      c.MAU_SAC_ID, 
      c.ID_KICH_CO, 
      c.SOLUONG_SANPHAM_CHITIET,
      m.TEN_MAU_SAC,        -- Thêm tên màu sắc từ bảng MAU_SAC
      k.KICH_CO              -- Thêm kích cỡ từ bảng KICH_CO
    FROM SAN_PHAM p
    INNER JOIN SAN_PHAM_CHI_TIET c ON p.ID_SAN_PHAM = c.ID_SAN_PHAM
    INNER JOIN MAU_SAC m ON c.MAU_SAC_ID = m.MAU_SAC_ID  -- Thêm JOIN với MAU_SAC
    INNER JOIN KICH_CO k ON c.ID_KICH_CO = k.ID_KICH_CO  -- Thêm JOIN với KICH_CO
    WHERE c.SOLUONG_SANPHAM_CHITIET < 20
  `;
